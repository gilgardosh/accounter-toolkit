...
export class monthExpansionRecordsHandler {
  ...
  public handle = async (): Promise<ReportInputRecord[]> => {
    try {
      ...
      const recordsTable = await waitForSelectorPlus(this.page, '#ContentUsersPage_dgHeshboniot');
  ...
  private getRecordDetails = async (index: number): Promise<ReportInputRecordDetails | undefined> => {
    try {
      if (!this.page) {
        this.page = await newPageByMonth(this.config.visibleBrowser, this.location[0], this.index);
      ...
      await waitAndClick(this.page, `#ContentUsersPage_dgHeshboniot_btnPratimNosafim_${index}`);
      ...
      this.prompt.addError([...this.location, 'Details'], (e as Error)?.message || e);
      return;
    }
  };
}
